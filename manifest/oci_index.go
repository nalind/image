...
// Instances returns a index of the manifests that this index knows of.
func (index *OCI1Index) Instances() []types.BlobInfo {
	results := make([]types.BlobInfo, len(index.Manifests))
	for i, m := range index.Manifests {
		results[i] = types.BlobInfo{
			Digest:      m.Digest,
			Size:        m.Size,
			MediaType:   m.MediaType,
			Annotations: dupStringStringMap(m.Annotations),
...
func (index *OCI1Index) UpdateInstances(updates []ManifestListUpdate) error {
	...
	for i := range updates {
		...
		if updates[i].MediaType == "" {
			return errors.Errorf("update %d of %d passed to OCI1Index.UpdateInstances had no media type", i+1, len(updates))
		}
...
func (index *OCI1Index) ImageID() string {
	return computeListID(index)
}
