...
var expectedCompressionFormats = map[string]*compression.Algorithm{
	...
	manifest.DockerV2Schema2ForeignLayerMediaTypeGzip: &compression.Gzip,
}
...
func (c *copier) copyBlobFromStream(ctx context.Context, srcStream io.Reader, srcInfo types.BlobInfo,
	...
	if expectedCompressionFormat, known := expectedCompressionFormats[srcInfo.MediaType]; known && compressionFormat.Name() != expectedCompressionFormat.Name() {
		logrus.Warnf("blob %s with type %s should be compressed with %s, but compressor appears to be %s", srcInfo.Digest.String(), srcInfo.MediaType, compressionFormat.Name(), expectedCompressionFormat.Name())
	}
